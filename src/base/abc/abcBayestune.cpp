...
string abc_stats_commmands(int h, int i, int which_opt)
{
    if (which_opt == 0 || which_opt == 1) //AIG minimization
        ...
        return ";strash;ifraig;scorr;dc2;strash;dch -f;if -K 6;mfs2;lutpack -S 1;print_stats -l;\" | grep \"nd =\" -A 2 > " + to_string(h)+"_"+to_string(i)+".result\n";
    ...
    else if(which_opt == 10) // Standard-cell LIB Technology mapping tuning
        return ";strash;ifraig;scorr;dc2;strash;dch -f;if -K 6;mfs2;lutpack -S 1;trace ;\" | grep \" =\" > " + to_string(h)+"_"+to_string(i)+".result\n";
    else 
...
float get_FPGA_level_from_result(string resultFile)
{
    ...
    int find_lat = input.find("lat =", 0);
    int find_luts = input.find("lut =", 0);
    string aig;
    if(find_lat>0)
    {
        aig.assign(input, find_lat+5, find_luts - find_lat - 5);
        return stof(aig);
    }
    else{ return 0.0; } //error message
    return 0.0;
...
extern "C" void bayes_flow_tune(char* Design, int repeats, int prefix_pos, int target, int nSamples, 
